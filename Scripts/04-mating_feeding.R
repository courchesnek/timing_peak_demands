...
food_labels <- c(
  ...
  "on_midden_mushroom" = "On Midden: Mushroom/Truffles",
  "off_midden_cone" = "Off Midden: Cone",
  "off_midden_mushroom" = "Off Midden: Mushroom/Truffles",
  "off_midden_animal_material" = "Off Midden: Animal Material",
  "off_midden_spruce_bud" = "Off Midden: Spruce Bud",
  "off_midden_other" = "Off Midden: Other")

ggplot(feeding_mating_summary, aes(x = "", y = count, fill = food_group)) +
  geom_bar(stat = "identity", width = 1) +  
  coord_polar(theta = "y") + 
  geom_text(aes(label = percent(proportion)), position = position_stack(vjust = 0.5), color = "black", size = 5) +
  scale_fill_manual(values = food_colors, labels = food_labels) +
  labs(title = "Food Types Consumed by Males During Mating", fill = "Food Group") +
  theme_void() +                                     
